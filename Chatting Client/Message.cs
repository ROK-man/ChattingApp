...
namespace Chatting_Client
{
    internal class Message
    {
        ...
        public static Message MakeChattingMessage(string payload)
        {
            ChattingMessage message = new(payload);
            return PackagePayload(MessageType.Chatting, (byte)Chatting.All, message);
        }

        private static Message PackagePayload(MessageType type, byte flag, MessagePayload payload)
        {
    ...
    internal class ChattingMessage : MessagePayload
    {
        ...
        public override void GetBytes(byte[] buffer, int offset)
        {
            ...
            int payloadLength = Math.Min(payloadBytes.Length, Message.MAX_PAYLOAD_SIZE);
